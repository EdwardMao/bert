...
from utils import START_TAG, STOP_TAG, BEGIN_TAG, MIDDLE_TAG, END_TAG, SINGLE_TAG
...
class SimpleLSTM(nn.Module):
    ...
    def __init__(self, device, vocab_size, nb_labels, emb_dim, hidden_dim):
        super().__init__()
...
class CRF(nn.Module):
    ...
    def __init__(self, device, nb_labels, tag_to_ix, batch_first=True):
        super().__init__()

        self.nb_labels = nb_labels
        self.BOS_TAG_ID = tag_to_ix[START_TAG]
        self.EOS_TAG_ID = tag_to_ix[STOP_TAG]
        self.BEGIN = tag_to_ix[BEGIN_TAG]
        self.MIDDLE = tag_to_ix[MIDDLE_TAG]
        self.END = tag_to_ix[END_TAG]
        self.SINGLE = tag_to_ix[SINGLE_TAG]
    ...
    def init_weights(self):
        ...
        # to begin
        self.transitions.data[self.MIDDLE, self.BEGIN] = -10000.0
        self.transitions.data[self.BEGIN, self.BEGIN] = -10000.0
        self.transitions.data[self.BEGIN, self.SINGLE] = -10000.0
        self.transitions.data[self.BEGIN, self.EOS_TAG_ID] = -10000.0
        # to middle
        self.transitions.data[self.BOS_TAG_ID, self.MIDDLE] = -10000.0
        self.transitions.data[self.END, self.MIDDLE] = -10000.0
        self.transitions.data[self.SINGLE, self.MIDDLE] = -10000.0
        self.transitions.data[self.MIDDLE, self.SINGLE] = -10000.0
        self.transitions.data[self.MIDDLE, self.EOS_TAG_ID] = -10000.0
        # to end
        self.transitions.data[self.BOS_TAG_ID, self.END] = -10000.0
        self.transitions.data[self.SINGLE, self.END] = -10000.0
        self.transitions.data[self.END, self.END] = -10000.0
        # to single
        # self.transitions.data[self.END, self.END] = -10000.0
        #
        # self.transitions.data[self.EOS_TAG_ID, :] = -10000.0
    ...
    def decode(self, emissions, mask=None, text=None):
        ...
        scores, sequences = self._viterbi_decode(emissions, mask, text)
    ...
    def _compute_scores(self, emissions, tags, mask):
        ...
        for i in range(1, seq_length):
            ...
            e_scores = emissions[:, i].gather(
                1, current_tags.unsqueeze(1)).squeeze()
    ...
    def _compute_log_partition(self, emissions, mask):
        ...
        alphas = self.transitions[self.BOS_TAG_ID,
    ...
    def _viterbi_decode(self, emissions, mask, text):
...
class BiLSTMCRF(nn.Module):
    def __init__(self, device, tag_to_ix, vocab_size, emb_dim=5, hidden_dim=4):
        super().__init__()
        self.nb_labels = len(tag_to_ix)
        self.lstm = SimpleLSTM(device, vocab_size, self.nb_labels, emb_dim=emb_dim, hidden_dim=hidden_dim)
        self.crf = CRF(device, self.nb_labels, tag_to_ix, batch_first=True)

    def forward(self, x, mask=None, text=None):
        emissions = self.lstm(x)
        score, path = self.crf.decode(emissions, mask=mask, text=text)
        return score, path
